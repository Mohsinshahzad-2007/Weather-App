        ...
        {weatherData && !loading && (
          <div>
            <div style={{ textAlign: "center", marginBottom: "25px" }}>
              ...
              <img src={getWeatherImage(weatherData.current.weather_code)} alt="Weather Status" style={{ width: "100px", height: "100px", borderRadius: "50%", objectFit: "cover", margin: "15px auto", display: "block", boxShadow: "0 4px 8px rgba(0,0,0,0.2)" }} />
              <h2 style={{ fontSize: "54px", margin: "10px 0", fontWeight: "300" }}>{weatherData.current.temperature_2m}°C</h2>
              <p style={{ fontSize: "18px", fontWeight: "bold", margin: "5px 0" }}>{getWeatherDescription(weatherData.current.weather_code)}</p>
              <p style={{ margin: 0, textTransform: "uppercase", letterSpacing: "1px", opacity: 0.9, fontSize: "14px" }}>Wind Speed: {weatherData.current.wind_speed_10m} km/h</p>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {weatherData.daily.time.slice(0, 3).map((time, idx) => (
                <div key={time} style={{ display: "flex", justifyContent: "space-between", background: "rgba(255,255,255,0.15)", padding: "12px 15px", borderRadius: "8px", fontSize: "14px" }}>
                  ...
                  <span style={{ fontWeight: "bold" }}>{weatherData.daily.temperature_2m_max[idx]}°C / {weatherData.daily.temperature_2m_min[idx]}°C</span>
                </div>
              ))}
            </div>
          </div>
